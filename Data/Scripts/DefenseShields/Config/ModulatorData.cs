...
namespace DefenseShields
{
    public class ModulatorState
    {
        ...
        public bool LoadState()
        {
            ...
            if (Modulator.Storage.TryGetValue(Session.Instance.ModulatorStateGuid, out rawData))
            {
                ...
                if (Session.Enforced.Debug >= 1) Log.Line($"Loaded - ModulatorId [{Modulator.EntityId}]:\n{State.ToString()}");
            }
    ...
    public class ModulatorSettings
    {
        ...
        public void NetworkUpdate()
        {
            ...
            if (Session.IsServer)
            {
                if (Session.Enforced.Debug >= 1) Log.Line($"ServRelay - ModulatorId [{Modulator.EntityId}]: network settings update for modulator");
                Session.PacketizeModulatorSettings(Modulator, Settings); // update clients with server's settings
            }
            else // client, send settings to server
            {
                if (Session.Enforced.Debug >= 1) Log.Line($"ClientRelay - ModulatorId [{Modulator.EntityId}]: sent network settings update for modulator");
