...
namespace DefenseShields.Station
{
    ...
    class DefenseShields : MyGameLogicComponent
    {
        ...
        public HashSet<IMyEntity> InHash = new HashSet<IMyEntity>();
        ...
        public override void UpdateBeforeSimulation()
        {
            try
            {
                ...
                if (_animInit)
                {
                    ...
                    MySimpleObjectDraw.DrawTransparentSphere(ref matrix, 300f, ref colour, MySimpleObjectRasterizer.SolidAndWireframe, 20, null, RangeGridResourceId, 0.15f, -1);
                    // end shield draw
                }
                if (Count % 3 == 0)
                {
        ...
        public void WebEffects()
        {
            ...
            if (Count == 0)
            {
                ...
                MyAPIGateway.Parallel.ForEach(InHash, outent =>
                {
                    ...
                    if (grid != null)
                    {
                        if (!InHash.Contains(outent)) InHash.Add(outent);
                    }
                });
                _insideReady = true;
            ...
            MyAPIGateway.Parallel.ForEach(playerHash, playerent =>
            {
                ...
                if (playerent is IMyCharacter)
                    ...
                    {   
                        ...
                        else if (!InHash.Contains(playerent)) InHash.Add(playerent);
                    }
                    catch (Exception ex)
                    {
