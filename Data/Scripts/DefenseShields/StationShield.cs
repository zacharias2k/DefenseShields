...
namespace DefenseShields.Station
{
    ...
    class DefenseShields : MyGameLogicComponent
    {
        ...
        public void ShowRange(float size)
        {
            ...
            MatrixD matrix = MatrixD.CreateFromTransformScale(Quaternion.CreateFromRotationMatrix(_worldMatrix2.GetOrientation()), _worldMatrix2.Translation, _scale);
        ...
        private bool Detectout(IMyEntity ent)
        {
            ...
            if (detect > 1)
            {
                if (detect > 8 || detect < 1) Logging.WriteLine(String.Format("{0} - {1} beyond-o-boundary cords: {2} {3} {4} {5}", DateTime.Now.ToString("MM-dd-yy_HH-mm-ss-fff"), ent, x, y, z, detect));
                return false;
            }
            return true;
        }
