...
*/
        ...
        public void ShowRange(float size)
        {
            ...
            _worldMatrix = Entity.GetWorldMatrixNormalizedInv();
            //_worldMatrix.Translation += Entity.WorldMatrix.Up * 0.35f;
        ...
        */
        ...
        private bool Detectout(IMyEntity ent)
        {
            ...
            if (detect > 1)
            {
                if (detect >10 || detect <1.05) Logging.WriteLine(String.Format("{0} - {1} beyond-t: x:{2} y:{3} z:{4} d:{5}", DateTime.Now.ToString("MM-dd-yy_HH-mm-ss-fff"), ent, x, y, z, detect));
                return false;
            }
