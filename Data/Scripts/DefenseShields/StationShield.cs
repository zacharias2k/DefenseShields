...
namespace DefenseShields.Station
{
    ...
    class DefenseShields : MyGameLogicComponent
    {
        ...
        public void ShowRange(float size)
        {
            ...
            var pos = _cblock.CubeGrid.GetPosition();
        ...
        private bool Detectout(IMyEntity ent)
        {
            ...
            if (detect > 1)
            {
                if (detect >8 || detect <1.2) Logging.WriteLine(String.Format("{0} - {1} beyond-t: x:{2} y:{3} z:{4} d:{5}", DateTime.Now.ToString("MM-dd-yy_HH-mm-ss-fff"), ent, x, y, z, detect));
                return false;
            }
