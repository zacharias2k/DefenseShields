...
namespace DefenseShields.Station
{
    ...
    class DefenseShields : MyGameLogicComponent
    {
        ...
        private bool Detectout(IMyEntity ent)
        {
            ...
            if (detect > 1)
            {
                if (detect > 9 || detect < 1.01) Logging.WriteLine(String.Format("{0} - {1} out-t: x:{2} y:{3} z:{4} d:{5} l:{6}", DateTime.Now.ToString("MM-dd-yy_HH-mm-ss-fff"), ent, x, y, z, detect, Count));
                return false;
            }
